...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
   ...
   if text == "Тестирование 📝":
   ...
   elif text in {"Нет", "Назад ◀"}:
       await update.message.reply_text("Возврат в меню", reply_markup=MAIN_MENU)
   elif text == "Тренажёр 🏋🏼":
       await update.message.reply_text('Выбери режим тренажёра', reply_markup=trainer_kb)
       ...
       idx = random.randint(0,len(SENTENCE_INCORRECT)-1)
       words = SENTENCE_INCORRECT[idx]
